...
you to use `set -e`.
You also shouldn't call these functions in any context where `set -e` is
suspended, such as in an `if` condition or on the left side of an `&&`
or `||` operator.

Unless otherwise noted, all library functions work fine with `set -u`,
`set -o pipefail`, and `shopt -s inherit_errexit`.
...
file.
The script will output code that defines the specified qsh functions
along with any other qsh functions needed as dependencies.
